...
pattern = re.compile(
    r'(ATG(?:[ATGC]{3})*?([ATGCZ*])*?(?:TAA|TAG|TGA))')  # ORF SEUQENCE
...
def frek(freq):
    fr1 = freq[2]+freq[3]
    fr2 = freq[0]+freq[1]+freq[2]+freq[3]
    ratio = (fr1) / (fr2)
    if (ratio <= 0.40 and ratio >= 0.60):
        return 0
    else:
        return 1
...
def primer(sequence, lmno):
    ...
    for j in range(20, 35):
        ...
        for i in range(0, len(sequence)):
            ...
            jpg = tm(atgc(k))
            if (sum == 7 and abs(jpg-t) < 5 and abs(jpg-t) > lmno % 4 and varble < 3):
                varble += 1
                string = "Forward Primer - " + \
                    str(k) + "\t" + "Tm(in deg C) - " + str(jpg)
                #print (string)
                op4_file.write(string)
                op4_file.write("\n")
                string = "Reverse Primer - " + \
                    str(seq) + "\t" + "Tm(in deg C) - " + str(tm(user))
                #print (string)
                op4_file.write(string)
                op4_file.write("\n")
                flag = 1
...
op3_file.write("> Protein Sequence")
op3_file.write("\n")
